...
class Crud_model extends CI_Model{
    ...
    public function listaMensagem($conexao) {
...
//Inserido por Diego Melo, a função insere o alerta de mensagem de rede pendente na tabela de alertas do portal
    public function insereMsgPortal($conexao_portal,$num_msg){
...
//Inserido por Diego Melo, a função verifica o numero de mensagens com o status repassado pela variavel $status
    public function verificaMsgStatus($status,$conexao){
        $query = "SELECT * FROM msg WHERE status={$status}";
        $result = mysqli_query($conexao,$query);
         return mysqli_num_rows($result);
    }

    public function atualizaStatus($conexao){
        $query = "UPDATE msg
                     SET status = '4'
    ...
    public function read_list($conexao) {
        $murais = array();
    ...
    public function create($conexao, $titulo, $mensagem, $assinatura, $observacao, $data_inicio, $data_final, $usuario_post, $data_post, $usuario_avaliador, $data_avaliada, $status, $ip, $tipo,$imediata) {
    ...
    public function update($conexao, $id, $titulo, $mensagem, $assinatura, $observacao, $data_inicio, $data_final, $ip, $tipo, $usuario_post, $data_post, $usuario_avaliador, $data_avaliada, $status,$imediata) {
    ...
    public function read($conexao, $id) {
        $query = "SELECT * FROM msg WHERE id = {$id}";
        $resultado = mysqli_query($conexao, $query);
        return mysqli_fetch_assoc($resultado);
    }

    public function delete($conexao, $id) {
